...
Optionally subsamples the input to a target read count (e.g. to get ~20x
coverage per condition from a 100x pbsim3 output).
...
complement of the segment immediately preceding the fold. The result still
looks like one continuous read, exactly as it would if the sequencer
produced it directly.

FOLD POSITIONS (all three preserve the input read length)
----------------------------------------------------------
  middle       Fold at ~50% of the read. Symmetric palindrome. Easy case
               for alignment-based detectors.
  off_center   Fold at ~75% of the read. Asymmetric; RC tail is ~25% of
               the read. (Renamed from "quarter" -- a fold at 25% of the
               read is physically impossible in a length-preserving
               foldback because there aren't enough forward bases to RC.)
  near_end     Fold within the last 500 bp of the read. RC tail is tiny
               (50-500 bp); tests the alignment-based blind spot where
               short RC tails get soft-clipped instead of triggering a
               supplementary alignment.

The construction now guarantees that the OUTPUT read length equals the
INPUT read length (for fold_point >= L/2, which all three modes satisfy).
Cross-condition differences are due to fold position alone, not read length.
...
------------------------------------------------------------------------
    ...
    fold_position     middle / off_center / near_end for foldbacks,
                      empty for clean
    adapter_present   True / False for foldbacks, empty for clean
    source_locus      chr6 coordinates the original read came from,
                      pulled from the pbsim3 .maf file

OUTPUT NAMING
-------------
    truth_<pct>pct_<position>_<adapterflag>.tsv
    sim_<pct>pct_<position>_<adapterflag>.fastq
...
Before running the real adapter_present-vs-duplex-tools comparison,
...
FOLD_POSITION_CHOICES = ("middle", "off_center", "near_end")

# Fixed subsampling seed so all 9 conditions in a sweep see the same reads.
# Kept separate from --seed so changing --seed doesn't change the subsample.
SUBSAMPLE_SEED = 42
...
def reservoir_sample(iterator, k, rng):
    """
    Uniform sample of k items from an iterator in one pass (Algorithm R).
    Memory: O(k). For k=230000 reads at ~15kb each, ~3.5 GB of RAM.
    """
    reservoir = []
...
def choose_fold_point(seq_len, fold_position, rng):
    """
    Return the fold point (position within the OUTPUT read where the fold
    happens) for the given fold_position category. All three modes produce
    fold_point >= seq_len / 2, so make_foldback can preserve read length.

      middle       -> ~45-55% of read length
      off_center   -> ~70-80% of read length (RC tail is ~25%)
      near_end     -> seq_len minus 50-500bp (RC tail is 50-500bp)
    """
    if fold_position == "middle":
        frac = rng.uniform(0.45, 0.55)
        return max(1, int(seq_len * frac))
    elif fold_position == "off_center":
...
def make_foldback(seq, qual, fold_position, adapter_present, adapter_seq, rng):
    """
    Build a foldback read that preserves the input read length.

    For a fold_point p in a read of length L (where p >= L/2 by design):
      - Forward portion: seq[:p]                    (length p)
      - RC tail: revcomp of the last L-p bases      (length L-p)
      - Output = forward + [adapter] + RC tail
      - Output length = p + adapter_length + (L-p) = L + adapter_length
    ...
    tail_len = min(fold_point, L - fold_point)
    tail_source = first[-tail_len:]  # last tail_len bases immediately before fold
    tail_rc = revcomp(tail_source)
    junction = adapter_seq if adapter_present else ""
    new_seq = first + junction + tail_rc

    first_q = qual[:fold_point]
    tail_q = first_q[-tail_len:][::-1]  # mirror quality with the folded segment
    junction_q = "I" * len(junction)  # placeholder high Phred for adapter bases
    new_qual = first_q + junction_q + tail_q
...
# --------------------------------------------------------------------------
# Sanity checks (streaming; safe for large FASTQs)
# --------------------------------------------------------------------------

def run_sanity_checks(fastq_path, truth_path):
    """
    Streaming sanity checks:
      - row counts match between fastq and truth table
      - all read_ids unique in the truth table
      - fastq read_ids match truth table read_ids (as sets)
      - fold_position non-empty when is_foldback=True
      - adapter_present is True/False when is_foldback=True
      - fold_position empty when is_foldback=False
    ...
    with open(truth_path) as fh:
        ...
        for row in reader:
            n_truth += 1
            rid = row["read_id"]
            assert rid not in truth_ids, f"Duplicate read_id in truth table: {rid}"
            truth_ids.add(rid)

            if row["is_foldback"] == "True":
                n_foldback += 1
                assert row["fold_position"] not in ("", None), (
                    f"Row {rid} has is_foldback=True but empty fold_position."
                )
                assert row["adapter_present"] in ("True", "False"), (
                    f"Row {rid} has is_foldback=True but adapter_present is not True/False."
                )
            else:
                assert row["fold_position"] == "", (
                    f"Row {rid} has is_foldback=False but non-empty fold_position."
                )

    assert n_fastq == n_truth, (
        f"Row count mismatch: FASTQ has {n_fastq} reads, "
        f"truth table has {n_truth} rows."
    )
    assert fastq_ids == truth_ids, "read_id sets differ between FASTQ and truth table."

    print(f"[sanity check] PASSED: {n_truth} reads, {n_foldback} foldback, "
...
def main():
    ...
    ap.add_argument("--subsample_n", type=int, default=None,
                     help="If set, uniformly subsample this many reads from the source "
                          "FASTQ before simulating foldbacks (reservoir sampling with a "
                          "fixed seed so all conditions in a sweep see the same reads). "
                          "Example: --subsample_n 230000 for ~20x on chr6.")
    ...
    ap.add_argument("--fold_position", choices=FOLD_POSITION_CHOICES, required=True,
                     help="Where the fold sits in the OUTPUT read: middle (~50%%), "
                          "off_center (~75%%), or near_end (last 500bp). All three "
                          "preserve input read length.")
    ...
    if args.subsample_n:
        print(f"[subsample] reservoir-sampling {args.subsample_n} reads "
              f"(seed={SUBSAMPLE_SEED})...", file=sys.stderr)
        sub_rng = random.Random(SUBSAMPLE_SEED)
        reads = reservoir_sample(iter_fastq(args.source_fastq), args.subsample_n, sub_rng)
        print(f"[subsample] got {len(reads)} reads.", file=sys.stderr)
    else:
