#!/usr/bin/env bash
#SBATCH --job-name=sniffles2_no_filter_clean
#SBATCH --output=logs/sniffles2_no_filter_clean_%j.log
...
OUT_DIR=~/foldback/results/sniffles2_no_filter

BAM="${OUT_DIR}/clean_control_no_filter.sorted.bam"
VCF="${OUT_DIR}/clean_control_no_filter.vcf"
CALLS="${OUT_DIR}/calls_sniffles2_no_filter_clean_control.tsv"

echo "=== sniffles2_no_filter: clean_control (aligning) ==="
minimap2 -ax map-ont -Y --MD -t "${SLURM_CPUS_PER_TASK}" "${REF}" "${FASTQ}" \
    | samtools sort -@ "${SLURM_CPUS_PER_TASK}" -o "${BAM}" -
samtools index "${BAM}"

echo "=== sniffles2_no_filter: running (--minsvlen 0) ==="
sniffles --input "${BAM}" --vcf "${VCF}" --output-rnames --minsvlen 0 --threads "${SLURM_CPUS_PER_TASK}"

python3 - "${VCF}" "${TRUTH}" "${CALLS}" "sniffles2_no_filter" <<'PYEOF'
import re
import sys
